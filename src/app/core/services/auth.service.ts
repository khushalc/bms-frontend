...
export class AuthService extends BaseService {
  ...
  /**
   * POST /auth/change-password — self-serve. Server requires the
   * current password so a stolen session token alone can't hijack the
   * account. Returns 204 (empty body) on success.
   */
  changePassword(currentPassword: string, newPassword: string): Observable<void> {
    return this.http.post<void>(`${this.config.baseUrl}/auth/change-password`, {
  ...
  /**
   * POST /auth/reset-password — public endpoint (no bearer needed;
   * the token IS the credential). Consumes a one-time reset token
   * generated by an admin / committee / primary / co-applicant on
   * the family member's behalf.
   */
  resetPassword(token: string, newPassword: string): Observable<void> {
    return this.http.post<void>(`${this.config.baseUrl}/auth/reset-password`, {
      token,
      new_password: newPassword,
    });
  }
